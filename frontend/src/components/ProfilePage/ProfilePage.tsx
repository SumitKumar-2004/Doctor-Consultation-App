...
const ProfilePage = ({ userType }: ProfileProps) => {
  ...
  const handleInputChnage = (field: string, value: any) => {
    // strip any optional-chaining characters that sneaked in from callers
  ...
  const cleanDataForUpdate = (data: any, userType: string) => {
    const cleaned: any = {};
    ...
    if (data.dob) cleaned.dob = data.dob;
    ...
    if (userType === "patient") {
      ...
      if (
        ...
        cleaned.medicalHistory = data.medicalHistory;
      }
      if (
        ...
        cleaned.emergencyContact = data.emergencyContact;
      }
    } else if (userType === "doctor") {
      // Doctor specific fields
      if (data.about) cleaned.about = data.about;
      if (data.specialization) cleaned.specialization = data.specialization;
      if (data.category && data.category.length > 0)
        cleaned.category = data.category;
      if (data.qualification) cleaned.qualification = data.qualification;
      if (data.experience) cleaned.experience = data.experience;
      if (data.fees) cleaned.fees = data.fees;
      if (
        data.hospitalInfo &&
        (data.hospitalInfo.name ||
          data.hospitalInfo.address ||
          data.hospitalInfo.city)
      ) {
        cleaned.hospitalInfo = data.hospitalInfo;
      }
      if (
        data.availabilityRange &&
        (data.availabilityRange.startDate ||
          data.availabilityRange.endDate ||
          data.availabilityRange.excludedWeekdays.length > 0)
      ) {
        cleaned.availabilityRange = data.availabilityRange;
      }
      if (data.dailyTimeRanges && data.dailyTimeRanges.length > 0)
        cleaned.dailyTimeRanges = data.dailyTimeRanges;
      if (data.slotDurationMinutes)
        cleaned.slotDurationMinutes = data.slotDurationMinutes;
    }
  ...
  const handleSave = async () => {
    try {
      const cleanedData = cleanDataForUpdate(formData, userType);
  ...
  const formatDateForInput = (isoDate: string): string => {
    ...
    // use local date components to avoid UTC offset causing previous-day
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (val) {
                  // construct iso in local timezone to avoid shifting
                  const d = new Date(val);
                  const y = d.getFullYear();
                  const m = String(d.getMonth() + 1).padStart(2, "0");
                  const day = String(d.getDate()).padStart(2, "0");
                  handleInputChnage("dob", `${y}-${m}-${day}T00:00:00`);
                } else {
                  handleInputChnage("dob", "");
                }
              }}
              disabled={!isEditing}
              className="w-80"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
          ...
          <Input
            ...
            onChange={(e) => {
              // normalize date avoiding timezone offset
              const dateObj = e.target.valueAsDate;
              ...
              // when startDate changes, ensure endDate isn't before it
              handleInputChnage("availabilityRange.startDate", iso);
              if (
                formData.availabilityRange?.endDate &&
                formData.availabilityRange.endDate < iso
              ) {
                handleInputChnage("availabilityRange.endDate", iso);
              }
            }}
            disabled={!isEditing}
          />
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            type="date"
            min={
              formatDateForInput(formData.availabilityRange?.startDate) ||
              (() => {
                const d = new Date();
                const y = d.getFullYear();
                const m = String(d.getMonth() + 1).padStart(2, "0");
