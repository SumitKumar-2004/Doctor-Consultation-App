...
const DoctorOnboardingForm = () => {
  ...
  // generic form field updater that supports dot notation for nested objects
  const handleInputChnage = (
    ...
    if (name.includes(".")) {
      ...
      setFormData((prev: DoctorFormData) => {
        ...
        // if we changed startDate and there's an endDate earlier, bump it
        if (
          name === "availabilityRange.startDate" &&
          updated.availabilityRange?.endDate &&
          // compare normalized YYYY-MM-DD strings
          normalizeToDateString(updated.availabilityRange.endDate) <
            normalizeToDateString(value)
        ) {
          updated.availabilityRange.endDate = value;
        }
        return updated;
      });
    } else {
  ...
  // Ensure strings used in <input type="date"> are plain YYYY-MM-DD
  const normalizeToDateString = (v?: string | null): string => {
    if (!v) return "";
  ...
  const handleSubmit = async (): Promise<void> => {
    try {
      await updateProfile({
        ...
        // availability dates stored as simple YYYY-MM-DD strings
        availabilityRange: {
