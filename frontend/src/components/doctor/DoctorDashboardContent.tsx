...
const DoctorDashboardContent = () => {
  ...
  const todayAppointmentsFromDashboard = dashboardData?.todayAppointments || [];
  const additionalFromUpcoming = (
    dashboardData?.upcomingAppointments || []
  ).filter(
    (a: Appointment) =>
      isSameLocalDay(a.slotStartIso) &&
      !todayAppointmentsFromDashboard.some((t: Appointment) => t._id === a._id),
  );
  const todaysCombined = [
    ...todayAppointmentsFromDashboard,
    ...additionalFromUpcoming,
  ];
  ...
  const statsCards = [
    ...
    },
    {
      ...
      // show merged count so UI matches list even if backend stats are stale
      value: todaysCombined.length.toString(),
      icon: Calendar,
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="lg:col-span-2 hover:shadow-lg transition-shadow">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <Badge variant="secondary" className="ml-2">
                    {todaysCombined.length} appointments
                  </Badge>
                </CardTitle>
              ...
              <CardContent className="space-y-4">
                {todaysCombined.length > 0 ? (
                  todaysCombined.map((appointment: Appointment) => (
                    <div
                      key={appointment?._id}
