...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // ensure availabilityRange dates are simple ISO yyyy-mm-dd strings
      if (updated.availabilityRange) {
        const { startDate, endDate } = updated.availabilityRange;
        if (startDate) {
          updated.availabilityRange.startDate = new Date(startDate)
            .toISOString()
            .split("T")[0];
        }
        if (endDate) {
          updated.availabilityRange.endDate = new Date(endDate)
            .toISOString()
            .split("T")[0];
        }
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // We query by both the `date` field and the ISO string stored in slotStartIso
      // to avoid timezone mismatches: date is stored as midnight UTC while the
      // doctor may be in a different zone; slotStartIso contains the exact
      // appointment timestamp in ISO format.
      ...
      // Upcoming appointments -- same dual criteria, but strictly after today.
